...
serve(async (req) => {
  ...
  try {
    ...
    const { coachId, coachName, position, date, time, athleteName, parentEmail } = await req.json();
    logStep("Received booking request", { coachId, position, date, time, athleteName, parentEmail });
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${req.headers.get("origin")}/training?success=true&session=${coachId}`,
